...
(function (root) {
  ...
  // inteiro). Elementos dentro de <template> aninhado nao sao carimbados; checkDcMap pega isso.
  function mapDcTemplate(text) {
    ...
    while (i < end) {
      ...
      if (DC_NOT_ELEMENTS[tag]) { i = j; continue; }
      out.push({ tplId: out.length, tag: tag, attrNames: attrNames, styleRange: styleRange });
      i = j;
      if (DC_RAW_TEXT[tag]) {
