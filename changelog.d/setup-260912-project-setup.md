...
- npm atualizado nesta máquina para 11.19.1, com autorização. O 10.9.2 quebrava em `arborist#loadPeerSet` no grafo de peers do Vitest 4.x. `npm@latest` (12.0.2) foi recusado — exige Node `^22.22.2 || ^24.15.0 || >=26.0.0` e esta máquina está no 22.14.0; o npm 11 aceita `>=22.9.0`. Depois disso `npm install` limpo passa e `npm audit` reporta 0 vulnerabilidades, fechando o item de dependências do checklist F.
